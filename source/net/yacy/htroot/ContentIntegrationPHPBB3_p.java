...
public class ContentIntegrationPHPBB3_p {
    ...
    public static serverObjects respond(@SuppressWarnings("unused") final RequestHeader header, final serverObjects post, final serverSwitch env) {
        ...
        if (post != null) {
            ...
            final String urlstub = post.get("content.phpbb3.urlstub", "");
            final String dbtype = post.get("content.phpbb3.dbtype", "");
            final String dbhost = post.get("content.phpbb3.dbhost", "");
            final int    dbport = post.getInt("content.phpbb3.dbport", 3306);
            final String dbname = post.get("content.phpbb3.dbname", "");
            final String prefix = post.get("content.phpbb3.tableprefix", "");
            final String dbuser = post.get("content.phpbb3.dbuser", "");
            final String dbpw = post.get("content.phpbb3.dbpw", "");
            final int    ppf = post.getInt("content.phpbb3.ppf", 1000);
            final String dumpfile = post.get("content.phpbb3.dumpfile", "");
            ...
            if (post.containsKey("import")) {
            	final File f = new File(dumpfile);
            	if (!f.exists()) {
            		prop.put("import", 2);
                    prop.put("import_error", "file " + dumpfile + " does not exist");
            	} else try {
                	final ImportDump importer = new ImportDump(
                                            dbtype,
                                            dbhost,
                                            dbport,
                                            dbname,
