...
public class imagetest {
    ...
    public static RasterPlotter respond(@SuppressWarnings("unused") final RequestHeader header, @SuppressWarnings("unused") final serverObjects post, @SuppressWarnings("unused") final serverSwitch env) {
        ...
        for (int y = 0; y < 600; y = y + 50) PrintTool.print5(img, 0, 6 + y, 0, Integer.toString(y), -1, 100);
        for (int x = 0; x < 800; x = x + 50) PrintTool.print5(img, x, 6    , 0, Integer.toString(x), -1, 100);
        ...
        img.setColor(RasterPlotter.GREY);
        PrintTool.print5(img, 50, 110, 0, "BROADCAST MESSAGE #772: NODE %882 GREY abcefghijklmnopqrstuvwxyz", -1, 100);
        img.setColor(RasterPlotter.GREEN);
        PrintTool.print5(img, 50, 120, 0, "BROADCAST MESSAGE #772: NODE %882 GREEN abcefghijklmnopqrstuvwxyz", -1, 100);
        ...
        for (byte c = (byte) 'A'; c <= 'Z'; c++) {
            ...
            PrintTool.arcPrint5(img, 550, 400, 100, angle, "ANGLE" + angle + ":" + (char) c, 100);
        }
        return img;
