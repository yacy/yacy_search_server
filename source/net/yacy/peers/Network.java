...
public class Network {
    ...
    public final void publishSeedList() {
        ...
        */
        ...
        Seed seed = this.sb.peers.mySeed();
        if ((seed.getIPs().contains(this.sb.peers.lastSeedUpload_myIP))
        ...
        if (!seedUploadMethod.equalsIgnoreCase("none") || seedUploadMethod.equals("") && this.sb.getConfig("seedFilePath", "").length() > 0) {
    ...
    protected final class publishThread implements Runnable {
        private final Seed seed;

        publishThread(final ThreadGroup tg, final Seed seed) {
            this.seed = seed;
        }

        @Override
        public final void run() {
            Thread.currentThread().setName("PublishSeed_" + seed.getName());
            ...
            try {
                ...
                Seed mySeed = Network.this.sb.peers.mySeed();
                long myLastSeenUTC = this.seed.getLastSeenUTC();

                for (final String ip : this.seed.getIPs()) {
                    try {
                        MultiProtocolURL targetBaseURL = this.seed.getPublicMultiprotocolURL(ip, preferHttps);

                        result = Protocol.hello(mySeed, Network.this.sb.peers.peerActions, targetBaseURL, this.seed.hash);
                        if (result == null && targetBaseURL.isHTTPS()) {
                            ...
                            result = Protocol.hello(mySeed, Network.this.sb.peers.peerActions,
                    ...
                    if (newSeed != null) {
                        if (!newSeed.isOnline()) {
                            ...
                            long newSeedLastSeen = newSeed.getLastSeenUTC();
                            if (newSeedLastSeen < (System.currentTimeMillis() - 10000)) {
                                // update last seed date

                                if (newSeedLastSeen >= myLastSeenUTC) {
                                    if (log.isFine()) {
                                        final String newSeedLastSeenStr = GenericFormatter.formatSafely(
                                                newSeedLastSeen, GenericFormatter.FORMAT_SHORT_SECOND);
    ...
    private boolean publishMySeed() {
        ...
        Seed mySeed = this.sb.peers.mySeed();
        if (mySeed.get(Seed.PEERTYPE, Seed.PEERTYPE_VIRGIN).equals(Seed.PEERTYPE_VIRGIN)) {
        ...
        try {
            ...
            if (record == null) {
                mySeed.put(Seed.NEWS, "");
            } else {
                mySeed.put(Seed.NEWS, net.yacy.utils.crypt.simpleEncode(record.toString()));
            }
        } catch (final Exception e) {
            log.severe("publishMySeed: problem with news encoding", e);
        }
        mySeed.setUnusedFlags();
        //if (seeds.length > 1) {
        ...
        String mySeedHash = mySeed.hash;

        seeds.values().stream().filter(seed ->
                seed != null && !seed.hash.equals(mySeedHash))
                .map(seed -> {
...
//                if ( logFine )
//                    log.fine("HELLO #" + Thread.currentThread(). + " to peer '" + seed.getName() + "' at " + address); // debug
                    ...
                    return new publishThread(Network.publishThreadGroup, seed);

                }).filter(Objects::nonNull).parallel().forEach(publishThread::run);
        ...
        if ((accessible + notaccessible) > 0) {
            ...
            if (mySeed.orVirgin().equals(newPeerType)) {
                log.info("PeerPing: myType is " + mySeed.orVirgin());
            } else {
                log.info("PeerPing: changing myType from '" + mySeed.orVirgin() + "' to '" + newPeerType + "'");
                mySeed.put(Seed.PEERTYPE, newPeerType);
            }
        } else {
            log.info("PeerPing: No data, staying at myType: " + mySeed.orVirgin());
        }
        ...
        if (mySeed.isProper(true) == null) return true;
        ...
        if (mySeed.get(Seed.PEERTYPE, Seed.PEERTYPE_JUNIOR).equals(Seed.PEERTYPE_JUNIOR)) {
            mySeed.put(Seed.PEERTYPE, Seed.PEERTYPE_SENIOR); // to start bootstraping, we need to be recognised as PEERTYPE_SENIOR peer
        }
        log.info("publish: no recipient found, our address is " + mySeed.getIPs());
        this.sb.peers.saveMySeed();
        return false;
//        }
